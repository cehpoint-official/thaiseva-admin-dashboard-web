...
const CarBookings = () => {
  const { carBookings, setRefetch, loadingData } = useContext(PartnerContext);
  ...
  useEffect(() => {
    setMatchedBookings(carBookings);
  }, [carBookings]);
  ...
  const handleViewBookingDetails = async (id) => {
    const snapshot = await getDoc(doc(carBookingsCollection, id));
    if (snapshot.exists()) {
  ...
  const handleViewPackageDetails = async (id) => {
    const snapshot = await getDoc(doc(carDetailsCollection, id));
    if (snapshot.exists()) {
      const existingData = snapshot.data();
  ...
  const handleDelete = (id) => {
    askingForDelete().then(async (result) => {
      if (result.isConfirmed) {
        await deleteDoc(doc(carBookingsCollection, id)).then(() => {
          setRefetch((p) => !p);
  ...
  const handleConfirmBooking = async () => {
    setError({ for: "", text: "" });
    if (bookingDetails?.driverName.length < 1) {
      return setError({ for: "driverName", text: "Invalid driver name" });
    }
    // else if (
    //   bookingDetails.driverPhoneNumber >= 11 ||
    //   bookingDetails.driverPhoneNumber < 8
    // ) {
    //   return setError({ for: "driverPhone", text: "Invalid driver phone" });
    // }
    await updateDoc(
      doc(carBookingsCollection, bookingDetails.orderId),
      bookingDetails
    ).then(() => {
      successNotification("Booking confirmed successfully.");
                            ...
                            {((value === "Pending" ||
                              ...
                              (column.property === "action" && (
                                <div className="flex gap-1 justify-center">
                                  ...
                                  <button
                                    onClick={() =>
                                      handleViewBookingDetails(row.orderId)
                                    }
        ...
        <Box>
          ...
          <DialogContent dividers sx={{ p: { xs: 2, md: 2 } }}>
            <Grid container spacing={2} sx={{ mb: 2, mt: 0.5 }}>
              ...
              {textField(
                "Check In",
                `${dateCalculator(bookingDetails?.pickUpTime)} ${timeCalculator(
              ...
              <Grid item xs={12} sm={6}>
                <TextField
                  ...
                  type="text"
                  placeholder=""
                  defaultValue={bookingDetails.driverName}
                  onChange={(e) =>
                    setBookingDetails((p) => ({
                      ...p,
              ...
              <Grid item xs={12} sm={6}>
                <TextField
                  ...
                  type="text"
                  placeholder=""
                  defaultValue={bookingDetails.driverPhoneNumber}
                  onChange={(e) =>
                    setBookingDetails((p) => ({
                      ...p,
