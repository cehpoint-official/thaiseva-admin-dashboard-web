...
// import CloseIcon from '@mui/icons-material/Home';
...
const HomeBanners = () => {
  ...
  useEffect(() => {
    const loadBanners = () => {
      const unSub = onSnapshot(
        query(settingsCollection, where("id", "==", "homeBanners")),
        (result) => {
  ...
  const handleAddBanner = async () => {
    ...
    // setBanners((p) => ({ ...p, id: uuid() }));
    if (banner.id) {
      await setDoc(doc(settingsCollection, "homeBanners"), {
        id: "homeBanners",
  ...
  const handleUpdateBanner = async () => {
    ...
    await updateDoc(doc(settingsCollection, "homeBanners"), {
      banners: banners,
    }).then(() => {
  ...
  return (
    <div className="mb-2 bg-slate-200 p-2 rounded">
      <div className="text-right mb-4 flex items-center justify-between h-full border-b border-[var(--primary-bg)] pb-2">
        <h2 className="banner-section-title">Home page banners</h2>
              ...
              <div
                ...
                className={`flex flex-row items-center justify-between border-2 p-1 rounded shadow-md shadow-black/30 ${
                  activeBanner?.i === i
                    ...
                    : "bg-white border-[var(--secondary-bg)]"
                }`}
              >
                <div className="flex gap-2 grow">
                  ...
                  <div className="relative grow">
                    ...
                    <span className="text-[10px] text-slate-600 p-0.5 rounded-sm bg-slate-200 border border-[var(--secondary-bg)] absolute top-2 right-2">
                      {banner.category}
                    </span>
                  </div>
                </div>
              ...
              <Grid item xs={12}>
                <FormControl fullWidth>
                  ...
                  <Select
                    ...
                    label="Select banner category"
                    value={banner.category}
                    onChange={(e) =>
                      setBanner((p) => ({ ...p, category: e.target.value }))
                    }
                  >
                    <MenuItem value="Food">Food</MenuItem>
                    <MenuItem value="Travel">Travel</MenuItem>
                    <MenuItem value="Hotel">Hotel</MenuItem>
                    <MenuItem value="Local_Assistance">
                      Local_Assistance
                    </MenuItem>
                  </Select>
                </FormControl>
