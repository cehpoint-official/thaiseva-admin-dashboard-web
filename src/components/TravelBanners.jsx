...
const TravelBanners = () => {
  ...
  useEffect(() => {
    const loadBanners = () => {
      ...
      const unSub = onSnapshot(
        query(settingsCollection, where("id", "==", "Travel")),
        (result) => {
  ...
  const handleAddBanner = async () => {
    ...
    if (banner.id) {
      await setDoc(doc(settingsCollection, "Travel"), {
        id: "Travel",
  ...
  const handleUpdateBanner = async () => {
    ...
    await updateDoc(doc(settingsCollection, "Travel"), {
      banners: banners,
    }).then(() => {
  ...
  return (
    <div className="mb-2 bg-slate-200 p-2 rounded">
      <div className="text-right mb-4 flex items-center justify-between h-full border-b border-[var(--primary-bg)] py-2 ">
        <h2 className="banner-section-title">Travel page banners</h2>
