...
const HotelBanners = () => {
  ...
  useEffect(() => {
    const loadBanners = () => {
      ...
      const unSub = onSnapshot(
        query(settingsCollection, where("id", "==", "Hotel")),
        (result) => {
  ...
  const handleAddBanner = async () => {
    ...
    if (banner.id) {
      await setDoc(doc(settingsCollection, "Hotel"), {
        id: "Hotel",
  ...
  const handleUpdateBanner = async () => {
    ...
    await updateDoc(doc(settingsCollection, "Hotel"), {
      banners: banners,
    }).then(() => {
  ...
  return (
    <div className="mb-2 bg-slate-200 p-2 rounded">
      <div className="text-right mb-4 flex items-center justify-between h-full border-b border-[var(--primary-bg)] py-2 ">
        <h2 className="md:text-xl text-lg font-semibold border-[var(--primary-bg)]">
          Hotel page banners
        </h2>
        ...
        <div className="flex-1 flex flex-col gap-3 bg-white border border-zinc-200 rounded p-2 ">
          {loading?.for === "data" && loading?.status === false ? (
            banners?.map((banner, i) => (
              <div
                key={i}
                className={`flex flex-row items-center justify-between border-2 border-zinc-200 p-1 rounded shadow-md shadow-black/30 ${
                  activeBanner?.i === i
              ...
              >
                ...
                <div className="flex gap-2 mr-1">
                  <div
                    onClick={() => {
                      setActiveBanner({ ...banner, i }), setIndex(i);
                    }}
                  >
                    <ViewDetailsIcon />
                  </div>
